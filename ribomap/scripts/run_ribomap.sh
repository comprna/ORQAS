...
if [ "${force}" = true ] || [ ! -f ${sm_out} ]; then
    ...
    salmon quant -r ${transcript_fa} ${ltype} -a ${rna_bam} -o ${sm_odir} -p $nproc --biasCorrect
    echo "salmon quant -t ${transcript_fa} ${ltype} -a ${rna_bam} -o ${sm_odir} -p $nproc --biasCorrect"
    head ${sm_out}
