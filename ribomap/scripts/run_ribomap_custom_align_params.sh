...
##############################################
#### Modified pipeline by Marina Reixachs ####
##############################################

#=============================
# cheat Salmon dir
#=============================
#SDIR="/home/mreixachs/brain_gonzalez/Salmon"
SDIR="/home/mreixachs/brain_gonzalez/validation/Salmon"
...
useRC=true # for rnaseq (salmon + riboprof)
min_fplen=27
max_fplen=32
...
align_reads ()
{
	...
	echo STAR --runThreadN $nproc --genomeDir ${transcript_idx} --readFilesIn $1 --outFileNamePrefix $2 ${SAM_params} ${filter_params}
...
##############################modified to read the bam without the undesired softclipped sequences
...
echo "align_reads" ${rna_nrrna_fa} ${ornaprefix} ${rna_bam}
echo "aligning ribo_seq to transcriptome..."
align_reads ${ribo_nrrna_fa} ${oriboprefix} ${ribo_bam}
echo "align_reads" ${ribo_nrrna_fa} ${oriboprefix} ${ribo_bam}

if [ ! -f  ${oriboprefix}Aligned.sorted.bam ];  then
	echo "samtools sort ${ribo_bam} ${oriboprefix}Aligned.sorted.bam" 
	samtools sort ${ribo_bam} ${oriboprefix}Aligned.sorted
fi

if [ ! -f  ${oriboprefix}Aligned.sorted.sam ];  then
	echo "samtools view -h ${oriboprefix}.sorted.bam > ${oriboprefix}Aligned.sorted.sam"
	samtools view -h ${oriboprefix}Aligned.sorted.bam > ${oriboprefix}Aligned.sorted.sam
fi

if [ ! -f  ${oriboprefix}Aligned.sorted.filtered.sam ];  then
	/home/mreixachs/brain_gonzalez/Ribomap_psite/filter_softclipping.pl ${oriboprefix}Aligned.sorted.sam  ${oriboprefix}Aligned.sorted.filtered.sam
fi

if [ ! -f  ${oriboprefix}Aligned.sorted.filtered.bam ];  then
	echo "samtools view -Sbh ${oriboprefix}Aligned.sorted.filtered.sam > ${oriboprefix}Aligned.sorted.filtered.bam"
	samtools view -Sbh ${oriboprefix}Aligned.sorted.filtered.sam > ${oriboprefix}Aligned.sorted.filtered.bam
fi

ribo_bam=${oriboprefix}Aligned.sorted.filtered.bam
...
#    ltype="-l SF"
#fi
echo "accessing salmon quant..."
sample="${rnaseq_fq##*/}";
#sample="${sample%%_*}";
sample="${sample%.*}";
mkdir -p ${sm_odir}
cp $SDIR/*/*/$sample*/* ${sm_odir}
#salmon quant -t ${transcript_fa} ${ltype} -a ${rna_bam} -o ${sm_odir} -p $nproc --biasCorrect
echo moved $SDIR/*/*/$sample*/ to ${sm_odir}
head ${sm_out}
